...
test.describe('ユーザーがタグでプレイヤーを検索できる#1', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('http://localhost:3001/ja', { timeout: 30000 } );
  })
  ...
  test('正常なタグを入力して検索して、該当するプレイヤーが表示されることを確認する#2', async ({ page }) => {
    ...
    await expect(page.locator('h1').getByText('neco3')).toBeVisible({ timeout: 30000 });
  });
  ...
  test('存在しないタグを入力して検索して、該当するプレイヤーが表示されないことを確認する#3', async ({ page }) => {
    ...
    await expect(page.getByText('No Player for: #AAAAAAAA')).toBeVisible({ timeout: 15000 });
  });
