...
test.describe('ユーザーがバトル履歴を見れるようにする#3', () => {
  test.beforeEach(async ({page}) => {
    await page.goto('http://localhost:3001/ja/players/Y2YPGCGC', { timeout: 15000  });
  })

  // バトル履歴のタイトルが表示される
  test('バトル履歴のタイトルが表示される', async ({ page}) => {
    await expect(page.getByText("バトル履歴")).toBeVisible({ timeout: 15000 })
  })
  ...
  test('バトル履歴の一人目のプレイヤーをタップすることでそのプレイヤーの詳細ページに遷移できる', async ({ page}) => {
    ...
    await link.click();
    await expect(page).toHaveURL(`http://localhost:3001/ja/players/${tag}`, { timeout: 15000 });
