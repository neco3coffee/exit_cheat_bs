...
const PlayerComponent = (player: any, starPlayerTag: string | null, battleType: string, isDuel?: boolean) => {
  const shortenedName = shortenPlayerName(player?.name);
  const isStarPlayer = player?.tag === starPlayerTag;
  const hashRemovedPlayerTag = player?.tag?.startsWith('#') ? player?.tag.slice(1) : player?.tag;

  return (
    <Link key={player?.tag} href={`/players/${hashRemovedPlayerTag}`} className={styles.playerContainer}>
      {isStarPlayer &&
