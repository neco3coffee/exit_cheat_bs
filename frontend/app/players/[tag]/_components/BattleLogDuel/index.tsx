...
const BattleLogDuel = ({
  ...
  const me = battleLog?.battle?.players.find((player: any) => player?.tag === `#${ownTag}`);
  const enemy = battleLog?.battle?.players.find((player: any) => player?.tag !== `#${ownTag}`);
  const isDuel = true;
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.middleContainer}>
        ...
        <h5 className={battleLog?.battle?.result === 'victory' ? styles.victory : styles.defeat}>{battleLog?.battle?.result}</h5>
        <div className={styles.right}>
          {
      ...
      <div className={styles.bottomContainer}>
        <div className={styles.bottomContainerInner}>
          {me && PlayerComponent(me, starPlayerTag, battleLog?.battle?.type, isDuel)}
          <div className={styles.vsContainer}>
            <strong>VS</strong>
            <Duration seconds={battleLog?.battle.duration} />
          </div>
          {enemy && PlayerComponent(enemy, starPlayerTag, battleLog?.battle?.type, isDuel)}
        </div>
      </div>
    </div>
  )
