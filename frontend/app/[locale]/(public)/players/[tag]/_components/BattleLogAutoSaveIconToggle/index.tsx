...
export default function BattleLogAutoSaveIconToggle({
  ...
  const toggleEnabled = async (newEnabled: boolean) => {
    try {
      const res = await fetch(`/api/v1/players/${tag}/auto_save`, {
        method: "POST",
        headers: {
  ...
  useEffect(() => {
    ...
    const updateRemaining = () => {
      ...
      if (days > 0) {
        setRemaining(`${days}d${hours % 24}h${minutes}m${seconds}s`);
      } else if (hours > 0) {
  ...
  return (
    <div
      className="flex items-center gap-5 justify-center mt-3! mb-3! border px-3 py-2! w-[270px]"
      style={{
